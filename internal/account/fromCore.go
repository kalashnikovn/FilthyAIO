...
type FromCoreBridgeAccount struct {
	Wallet
	Helper
}

func NewFromCoreBridgeAccount(wallet Wallet) FromCoreBridgeAccount {
	return FromCoreBridgeAccount{
		wallet,
		Helper{},
	}
}

func (acc FromCoreBridgeAccount) Bridge() {
	constants.Logger.Warn(fmt.Sprintf("Начинаю From CoreDao Bridge для аккаунта %s", acc.PublicKey.String()))
	utils.SendTelegramMessage(fmt.Sprintf("Начинаю From CoreDao Bridge для аккаунта %s", acc.PublicKey.String()))

	chain := []string{"coredao"}
	...
	swap, err := acc.FromCoreBridgeSwap(swapArgs)
	if err != nil {
		constants.Logger.WithFields(logrus.Fields{
			color.InRed("Ошибка"): err,
			//"Debug":  swapArgs,
...
func (acc FromCoreBridgeAccount) FromCoreBridgeSwap(args SwapArgs) (*types.Receipt, error) {
	chainFrom := "coredao"
	...
	contractAddress := common.HexToAddress(constants.COREDAO_BRIDGE_CONTRACT)
	contractAbi, err := utils.ReadAbi(constants.CORE_DAO_FROM_ABI)
	if err != nil {
		return nil, err
	}

	token := common.HexToAddress(constants.CONTRACTS["USDC"][chainFrom])
	dstChainId := uint16(102)
	//spew.Dump(token)
	...
	encodedData, err := contractAbi.Pack("bridge",
		token, dstChainId, amountLD, to, false, callParams, adapterParams)
	if err != nil {
		return nil, err
	}
...
func (acc FromCoreBridgeAccount) GetSwapValue() (*big.Int, error) {
	client := constants.CLIENTS["coredao"]
